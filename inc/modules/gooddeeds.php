...
class gooddeeds extends metamodule
{
	...
	function _showOne($bid)
    {
        ...
        if ($bid) {
            $block = $control->all->b_data_all($bid, 'gooddeeds');
            $control->name = $block->title;			
			
			$list = new Listing('gooddeeds_gallery', 'items', $bid);
			$list->get_list();
			$list->get_item();
    ...
    function page($arParams = array())
    {
        ...
        $list = new Listing('gooddeeds', 'blocks', 401);
        $list->get_list();
        $list->get_item();
		...
		foreach( $page->item as $item ) {			
			...
			$list = new Listing('gooddeeds_gallery', 'items', $item->id);
			$list->get_list();
			$list->get_item();
