...
abstract class elFinderVolumeDriver {
	 ...
	 * @param $string
	 * @return mixed
	 * @author Ruslan (maestroprog) Yarullin
	 */
	public function translit($string)
	{
		...
		if (substr_count($string, '.') > 0) {
            ...
            $string = implode('-', $string) . '.' . $ext;
        }
		return trim(str_replace('-.', '.', preg_replace(array('/[^a-zN0-9\-\.]/i', '/-{2,}/'), array('-', '-'), strtr(mb_strtolower($string, $this->encoding ? $this->encoding : 'UTF-8'), $converter))), '-');
	}
	...
	protected function mimetype($path, $name = '') {
		...
		$type = explode(';', $type);
		$type = trim($type[0]);
		if (!in_array($type, $this->uploadDeny) && strstr($type, 'text/')) {
			$_file = file_get_contents($path);
			if (stristr($_file, '<?')) $type = 'text/x-php';
			if (stristr($_file, 'eval')) $type = 'text/x-php';
			unset($_file);
		}
