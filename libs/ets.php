...
class _ets
{
		...
		for ( ; $i < $ncontent; ++$i) {
			...
			// cdata
				...
				// text acquisition
					...
					switch ($c0) {
						case "\n": $ctext .= "\1n\1"; break;
						case "\r": $ctext .= "\2r\2"; break;
						case "\t": $ctext .= "\3t\3"; break;
					    case " " : $ctext .= "\4s\4"; break;
	...
	{
		$data = str_replace("\n", "\1n\1", $data);
		$data = str_replace("\r", "\2r\2", $data);
		$data = str_replace("\t", "\3t\3", $data);
		return  str_replace(" " , "\4s\4", $data);
	}
	...
	function build_all($datatree, $entry)
	{
		...
		$built = str_replace("\2r\2", "\r", $built);
		$built = str_replace("\3t\3", "\t", $built);
		$built = str_replace("\4s\4", " ",  $built);
...
function cachename($filename){
    if (!is_dir('_cache')) mkdir('_cache', 0777, true);
    return '_cache' . DIRECTORY_SEPARATOR  . md5($filename) . '.ets';
}

function ets_cache_read_handler($filename)
{
...
function ets_cache_write_handler($filename, $data)
{
    $cachename = cachename($filename);
