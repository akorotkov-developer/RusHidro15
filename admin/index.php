...
if ($_SESSION['admin_redirect_url']) {
    $url = $_SESSION['admin_redirect_url'];
    $_SESSION['admin_redirect_url'] = '';
    header("Location:" . $url);
    exit;
}
...
$_SESSION['hidestructure'] = $hidestructure;
	...
	if (!$user_is_super) {
		$res = mysql_query("SELECT * FROM $prname"."_ctemplates tpl, $prname"."_categories cats WHERE cats.template=tpl.key ORDER BY cats.id");
	};

	if ((!$user_is_super) && (mysql_num_rows($res) > 0)) {
			mysql_data_seek($res, 0); $scaneditbl = '';
			while ($row = mysql_fetch_array($res)) {if ($row['canaddbl'] < 1) $scaneditbl .= ' && (id != '.$row['id'].')';};
	};
	$page->scaneditbl = $scaneditbl;

	if ((!$user_is_super) && (mysql_num_rows($res) > 0)) {
			mysql_data_seek($res, 0); $scandel = '';
			while ($row = mysql_fetch_array($res)) {if ($row['candel'] < 1) $scandel .= ' && (id != '.$row['id'].')';};
	};
	$page->scandel = $scandel;

	if ((!$user_is_super) && (mysql_num_rows($res) > 0)) {
			mysql_data_seek($res, 0); $scanaddcat = '';
			$scanaddcat .= ' && (id != 0)';
			while ($row = mysql_fetch_array($res)) {if ($row['canaddcat'] < 1) $scanaddcat .= ' && (id != '.$row['id'].')';};
	}; 
	$page->scanaddcat = $scanaddcat;

	if ((!$user_is_super) && (mysql_num_rows($res) > 0)) {
			mysql_data_seek($res, 0); $scanedit = '';
			while ($row = mysql_fetch_array($res)) {if ($row['canedit'] < 1) $scanedit .= ' && (id != '.$row['id'].')';};
	};
	$page->scanedit = $scanedit;

	if ((!$user_is_super) && (mysql_num_rows($res) > 0)) {
			mysql_data_seek($res, 0); $scanhide = '';
			while ($row = mysql_fetch_array($res)) {if ($row['canhide'] < 1) $scanhide .= ' && (id != '.$row['id'].')';};
	};
	$page->scanhide = $scanhide;

	if ((!$user_is_super) && (mysql_num_rows($res) > 0)) {
			mysql_data_seek($res, 0); $scancopy = '';
	...
	if (tree_count($tree) < 2) { $scancopy_22 = ' && false'; };
	$page->scancopy_22 = $scancopy_22;


	if ((!$user_is_super) && (mysql_num_rows($res) > 0)) {
			mysql_data_seek($res, 0); $scanmove = '';
			while ($row = mysql_fetch_array($res)) {
	...
	if (tree_count($tree) < 2) { $scanmove_22 = ' && false'; }; 
								...
								$page->item[$ii]->count = $count;
									...
									while ($r2 = tree_item($i++, $tree))
										...
										$page->item[$ii]->item2[$i]->r = $r;
											...
											if ($i < $r['lev']) {
												if ($l[$i] == 'closed') { $page->item[$ii]->item2[$i]->l = 'spacer';  }	else { $page->item[$ii]->item2[$i]->l = 'm4';   }
											} else {
												if ($l[$i] == 'closed') { $page->item[$ii]->item2[$i]->l = 'm2';  }		else  { $page->item[$ii]->item2[$i]->l = 'm1';  }
											};
...
$text = sprintt($page, 'templates/cats/welcome.html');
$text = str_replace('{base_url}', '', $text);
echo $text;
